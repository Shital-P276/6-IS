...
public class ExamServerGUI extends JFrame {
    ...
    public ExamServerGUI() {
        ...
        setTitle("IS Lab Exam Server - DES Secure QP Delivery");
        setSize(600, 400);
    ...
    private void handleClient(Socket client) {
        new Thread(() -> {
            try (BufferedReader in = new BufferedReader(new InputStreamReader(client.getInputStream()));
                ...
                appendLog("DH Shared Secret Established : " + sharedSecret);
                ...
                appendLog("QP encrypted with DES");

                // Send metadata
                out.println("OK");
                out.println(encryptedQP.length());
                out.println("3"); // Send the fixed key/shift so the client doesn't hang
                out.flush(); // CRITICAL: Flush text buffer before sending binary data
    ...
    // STUDENT CUSTOMIZE: Des Encryption (shift >0 for encrypt)
    private String desEncrypt(String text) {
        try {
            return DESUtil.encrypt(text);
        } catch (Exception e) {
