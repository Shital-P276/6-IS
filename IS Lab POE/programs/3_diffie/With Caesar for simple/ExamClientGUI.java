...
public class ExamClientGUI extends JFrame {
    ...
    private void requestQP() {
        new Thread(() -> {
            try (Socket socket = new Socket("localhost", 8080);
                ...
                out.println(clientPublic);  // fixed: println not print
                long serverPublic  = Long.parseLong(in.readLine());
                long sharedSecret  = DHUtil.computeSharedSecret(serverPublic, clientPrivate);
                qpArea.setText("Client Private Key : " + clientPrivate + "\n"
                             + "Client Public Value: " + clientPublic  + "\n"
                             + "Server Public Value: " + serverPublic  + "\n"
                             + "Shared Secret K    : " + sharedSecret  + "\n\n");
                ...
                SwingUtilities.invokeLater(() -> {
                    qpArea.append("Encrypted QP received (length: " + len + ")\nKey (shift): " + receivedShift + "\n\n" + encryptedQP);
    ...
    // STUDENT CUSTOMIZE: Caesar Decrypt using DH shared secret as shift
