...
public class ExamClientGUI extends JFrame {
    ...
    private void requestQP() {
        new Thread(() -> {
            try (Socket socket = new Socket("localhost", 8080);
                ...
                System.out.println("DH Shared Secret Established: " + sharedSecret);
    ...
    // STUDENT CUSTOMIZE: transpose Cipher Decryption (negative shift for decrypt)
    private String desDecrypt(String text) {
        try {
            // Ensure you are using the static method from your DESUtil class
            return DESUtil.decrypt(text);
        } catch (Exception e) {
            e.printStackTrace(); // Print the error to the console so you can see why DES failed
            return "DECRYPTION_ERROR: " + e.getMessage();
        }
    }
