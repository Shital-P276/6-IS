...
public class ExamClientGUI extends JFrame {
    ...
    private String receivedSignature; // STUDENT CUSTOMIZE: store hash for verification
    private String receivedPublicKey; // STUDENT CUSTOMIZE
    ...
    private void requestQP() {
        new Thread(() -> {
            try (Socket socket = new Socket("localhost", 8080);
                ...
                // STUDENT CUSTOMIZE: read hash sent by server
    ...
    private void decryptAndDownload() {
        try {
            ...
            // STUDENT CUSTOMIZE: verify hash after decryption
            byte[] keyBytes = Base64.getDecoder().decode(receivedPublicKey);
            PublicKey publicKey = KeyFactory.getInstance("RSA").generatePublic(new X509EncodedKeySpec(keyBytes));
            boolean valid = RSAUtil.verify(decrypted, receivedSignature, publicKey);

            if (valid) {
                qpArea.setText("✓ SIGNATURE VALID - QP is Authentic!\n\nDECRYPTED QP:\n\n" + decrypted);
            } else {
                qpArea.setText("✗ SIGNATURE INVALID - QP was Tampered!");
                return;
            }
