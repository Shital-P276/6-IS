...
public class ExamClientGUI extends JFrame {
    ...
    private String receivedHash; // STUDENT CUSTOMIZE: store hash for verification
    ...
    private void decryptAndDownload() {
        try {
            ...
            if (receivedHash.equals(computedHash)) {
                qpArea.setText("✓ HASH MATCH - QP is Authentic!\n\nReceived Hash : " + receivedHash
                        + "\nComputed Hash : " + computedHash + "\n\nDECRYPTED QP:\n\n" + decrypted);
            } else {
                qpArea.setText("✗ HASH MISMATCH - QP was Tampered!\n\nReceived Hash : " + receivedHash
                        + "\nComputed Hash : " + computedHash);
                return;
            }
