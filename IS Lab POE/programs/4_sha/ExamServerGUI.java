    ...
    private void handleClient(Socket client) {
        new Thread(() -> {
            try (BufferedReader in = new BufferedReader(new InputStreamReader(client.getInputStream()));
                ...
                // STUDENT CUSTOMIZE: compute hash for integrity check
                String hash = HashUtil.sha256(qpContent);
                appendLog("QP Hash (SHA-256): " + hash);
                ...
                // Send metadata
                out.println("OK");
                out.println(hash);               // send hash for client to verify
                ...
                dos.flush();
                appendLog("Hash + Encrypted QP sent to client");
